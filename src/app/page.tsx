...
export default function Home() {
  ...
  const [issueData, setIssueData] = useState<IdentityData>({ name: 'Alice', age: 25, residency: 'USA' });
  ...
  const [requestedFields, setRequestedFields] = useState<string[]>(['name', 'age', 'residency']); // Default all
  ...
  const sendID = () => {
    ...
    try {
      ...
      const fieldsToReveal = pendingRequest.length > 0 ? pendingRequest : ['age', 'residency', 'name'];
  ...
  const verifyID = () => {
    ...
    try {
      ...
      const isValid = IdentityManager.verifyPresentation(receivedPod, receivedIssuerPk);

      if (isValid) {
        setVerificationResult(true);
        setVerifierStatus('Cryptographically Verified & Checking Conditions...');
  ...
  const renderHome = () => (
    <div className={styles.step}>
      ...
      <div className={styles.card}>
        <button onClick={() => setMode('issuer')} className={styles.button}>GOVERNMENT (Issuer)</button>
        <p>Mint new Digital IDs</p>
      </div>
      <div className={styles.card}>
        <button onClick={() => setMode('holder')} className={styles.button}>CITIZEN (Holder)</button>
        <p>Store and Present ID</p>
      </div>
      <div className={styles.card}>
        <button onClick={() => setMode('verifier')} className={styles.button}>SERVICE (Verifier)</button>
  ...
  const renderIssuer = () => (
    <div className={styles.step}>
      <h3><button onClick={() => setMode('home')} className={styles.back}>←</button> Issuer Portal</h3>

      <div className={styles.form}>
        <label>Name: <input value={issueData.name} onChange={e => setIssueData({ ...issueData, name: e.target.value })} className={styles.input} /></label>
        <label>Age: <input type="number" value={issueData.age} onChange={e => setIssueData({ ...issueData, age: Number(e.target.value) })} className={styles.input} /></label>
        <label>Residency: <input value={issueData.residency} onChange={e => setIssueData({ ...issueData, residency: e.target.value })} className={styles.input} /></label>
        <button onClick={mintID} className={styles.button}>Mint Digital ID</button>
      </div>
          ...
          <button
            ...
            onClick={() => {
              try {
                ...
                const pres = IdentityManager.createPresentation(pod, ['name', 'age', 'residency']);
                console.log("Created Presentation:", pres);
                const valid = IdentityManager.verifyPresentation(pres, parsed.issuerPk);
                alert(valid ? "Local Verification PASSED" : "Local Verification FAILED (Check Console)");
              } catch (e) { console.error(e); alert("Error Checking: " + e); }
            }}
          >
            Debug: Verify Now
          </button>
          <p style={{ fontSize: '0.9em', marginTop: '10px' }}>User: Scan this to import ID</p>
          <button onClick={downloadPodFile} className={styles.button} style={{ background: '#666', fontSize: '0.8em', marginTop: '10px' }}>Download backup</button>
        </div>
      )}
    </div>
  );
  ...
  const renderHolder = () => (
    <div className={styles.step}>
      <h3><button onClick={() => setMode('home')} className={styles.back}>←</button> Citizen Wallet</h3>
      ...
      {!myPod ? (
        <div>
          ...
          <div className={styles.card}>
            {!isScanning ? (
              <button onClick={() => setIsScanning(true)} className={styles.button}>📷 Scan QR Code</button>
            ) : (
              <div id="reader" style={{ width: '100%' }}></div>
            )}
          </div>
          {isScanning && <button onClick={() => setIsScanning(false)} className={styles.button} style={{ background: '#888' }}>Cancel Scan</button>}

          <p style={{ fontSize: '0.9em', marginTop: '20px' }}>Or upload backup:</p>
          <input type="file" accept=".pod,.json" onChange={handleFileUpload} className={styles.input} />
        </div>
      ) : (
        <div>
          <div className={styles.card} style={{ background: '#e3f2fd' }}>
            <h4>My Digital ID</h4>
            <p>ID Loaded: <strong>{myPod ? "Ready" : "Empty"}</strong></p>
          </div>
            ...
            <div>
              ...
              {pendingRequest.length > 0 ? (
                <div className={styles.card} style={{ borderColor: '#ff9800' }}>
                  <p><strong>Verifier Request:</strong></p>
                  <ul style={{ textAlign: 'left' }}>
                    {pendingRequest.map(f => <li key={f}>{f}</li>)}
                  </ul>
                ...
                <div>
                  ...
                  <button onClick={sendID} className={styles.button} style={{ opacity: 0.5 }}>Force Share All</button>
                </div>
              )}
  ...
  const renderVerifier = () => (
    <div className={styles.step}>
      <h3><button onClick={() => setMode('home')} className={styles.back}>←</button> Service Verifier</h3>
        ...
        <div className={styles.card}>
          ...
          <div style={{ textAlign: 'left', margin: '0 auto', maxWidth: '300px' }}>
            {['name', 'age', 'residency'].map(f => (
              <div key={f} style={{ marginBottom: '8px', borderBottom: '1px solid #eee' }}>
                <label style={{ display: 'block', fontWeight: 'bold' }}>
                  <input type="checkbox" checked={requestedFields.includes(f)} onChange={() => toggleRequest(f)} /> {f}
                </label>
                {requestedFields.includes(f) && (
                  <div style={{ marginLeft: '25px', display: 'flex', alignItems: 'center', gap: '5px', marginBottom: '5px' }}>
                    <span style={{ fontSize: '0.8em' }}>Req:</span>
                    <select
                      style={{ fontSize: '0.8em' }}
                      value={requirements[f]?.op || '=='}
                      onChange={(e) => setRequirements(p => ({ ...p, [f]: { op: e.target.value as any, val: p[f]?.val || '' } }))}
                    >
                      <option value="==">==</option>
                      <option value=">=">{'>='}</option>
                      <option value="<=">{'<='}</option>
                    ...
                    <input
                      style={{ fontSize: '0.8em', width: '80px', padding: '2px' }}
                      placeholder="Value"
                      value={requirements[f]?.val || ''}
                      onChange={(e) => setRequirements(p => ({ ...p, [f]: { op: p[f]?.op || '==', val: e.target.value } }))}
                    />
                  </div>
                )}
              </div>
          ...
          <button onClick={() => app.host()} className={styles.button}>Create Session</button>
        </div>
      )}
      ...
      {step === 2 && (
        <div>
          ...
          <div
            ...
            style={{ padding: '15px', background: '#f5f5f5', wordBreak: 'break-all', userSelect: 'all', cursor: 'text' }}
          >
            {typeof window !== 'undefined' ? `${window.location.origin}?code=${joiningCode}` : joiningCode}
          </div>
          <p style={{ fontSize: '0.9em', color: '#666' }}>Or enter code manually: <strong>{joiningCode}</strong></p>
      ...
      {step === 3 && (
        <div>
          ...
          {receivedPod && (
            <div className={styles.card}>
              <h4>ID Received</h4>
              <button onClick={verifyID} className={styles.button}>Verify Signature</button>
            </div>
          )}

          {verificationResult !== null && (
            <div className={styles.result}>
              <div style={{ color: verificationResult ? 'green' : 'red', fontWeight: 'bold' }}>
                {verificationResult ? '✅ VALID SIGNATURE' : '❌ INVALID'}
              </div>

              {verificationResult && (
                <div style={{ marginTop: '10px', textAlign: 'left', background: '#eee', padding: '10px', borderRadius: '5px' }}>
                  <p><strong>Revealed Attributes:</strong></p>
                    ...
                    return (
                      <ul style={{ listStyle: 'none', paddingLeft: 0 }}>
                        {Object.keys(revealed).map(key => {
                          ...
                          return (
                            <li key={key} style={{ marginBottom: '5px', padding: '5px', borderBottom: '1px solid #ccc' }}>
                              <div>
                                <span style={{ textTransform: 'capitalize', width: '80px', display: 'inline-block' }}>{key}</span>
                                <strong>{val.value.toString()}</strong>
                                <span style={{ fontSize: '0.8em', color: 'green', marginLeft: '5px' }}> (Verified)</span>
                              </div>
                              {hasReq && (
                                <div style={{ fontSize: '0.9em', marginTop: '2px', color: reqPassed ? 'green' : 'red' }}>
                                  Condition: {requirements[key]?.op} {requirements[key]?.val}
                                  <strong style={{ marginLeft: '10px' }}>{reqPassed ? '✅ MET' : '❌ FAILED'}</strong>
                                </div>
                              )}
                            </li>
                          );
